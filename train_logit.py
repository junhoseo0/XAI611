...
# Training function
def train(model, dataloader, criterion, optimizer, adjustments, device):
    model.train()
    running_loss = 0.0
    for images, labels in tqdm(dataloader, desc="Training", leave=False):
        ...
        loss = criterion(outputs + adjustments, labels)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def main(args: list[str]):
    ...
    for epoch in range(num_epochs):
        train_loss = train(model, train_loader, F.cross_entropy, optimizer, adjustments, device)
