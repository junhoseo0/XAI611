...
# Training function
def train(model, dataloader, criterion, optimizer, adjustments, device, temperature=0.5, lamb=0.5):
    model.train()
    ...
    for images, labels in tqdm(dataloader, desc="Training", leave=False):
        ...
        loss = nt_xent_loss + lamb * criterion(logit + adjustments, labels.view(-1).to(device))
...
def main(args: list[str]):
    ...
    adjustments = torch.from_numpy(adjustments).to(device)
    ...
    for epoch in range(num_epochs):
        train_loss = train(
            model, train_loader, F.cross_entropy, optimizer, adjustments, device, lamb=100.0
        )
